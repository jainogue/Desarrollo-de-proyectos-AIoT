...
namespace app
{
    ...
    void AppAudio::feedTask(void *arg)
    {
        ...
        while (true)
        {
            ...
            // 1) Calcular el máximo absoluto en el bloque
            int16_t max_val = 0;
            ...
            vTaskDelay(pdMS_TO_TICKS(500));  // Delay para permitir el procesamiento
        }
    ...
    void AppAudio::actionTask(void *arg)
    {
        ...
        while (true)
        {
            if (xSemaphoreTake(xDetectSemaphore, portMAX_DELAY) == pdTRUE)
            {
                ...
                float scale = input_tensor->params.scale;   // Escala definida por el modelo
                int zero_point = input_tensor->params.zero_point; // Zero point definido por el modelo
                ...
                for (int i = 0; i < AUDIO_BUFFER_SIZE; i++) {
                    ...
                    float normalized = obj->m_features[i];
                    int quantized = static_cast<int>(std::round(normalized / scale)) + zero_point;
                    // Saturar el valor para que se mantenga en el rango int8
                    if (quantized > 127) quantized = 127;
                    if (quantized < -128) quantized = -128;
                    obj->m_input->data.int8[i] = static_cast<int8_t>(quantized);
                }
                ...
                {
                    int8_t *output = obj->m_interpreter->output(0)->data.int8;
                    TfLiteTensor *output_tensor = obj->m_interpreter->output(0);
                    float output_scale = output_tensor->params.scale;
                    int output_zero_point = output_tensor->params.zero_point;
                    float output_float[2];
                    for (int i = 0; i < 2; i++) {
                        output_float[i] = (output[i] - output_zero_point) * output_scale;
                    }
                    ...
                    //ESP_LOGI("actionTask", "Clase 0 (Cry): %.6f", output_float[0]);
                    //ESP_LOGI("actionTask", "Clase 1 (Noise): %.6f", output_float[1]);

                    // Notificar si hay un cambio en el estado
                    int result_idx = (output_float[0] > output_float[1]) ? 0 : 1;
                    obj->m_state_fn(result_idx, inference_time_ms, output_float[0], output_float[1], used_ram);
